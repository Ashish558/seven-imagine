...
export default function NavLink({width, icon: Icon, path, parentClassName }) {
   ...
   return (
      <button className={`lg:mb-14 z-20 relative ${styles.navLink} ${parentClassName ? parentClassName : ''} 
      ${location.pathname === path ? styles.selectedNavLink : ''}
      ${location.pathname === path && width < desktop ? styles.translateUp : '' } `} onClick={handleNavigate} >
         {/* <img src={icon} className={Selected ? styles.selectedNavLink : ''} /> */}
         <Icon />
         {location.pathname === path &&
            <img src={width > desktop ? Selected : SelectedMobile} className={styles.selectedIcon} />
         }
      </button>
   )
}
